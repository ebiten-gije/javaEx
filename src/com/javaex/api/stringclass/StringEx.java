...
public class StringEx {
	...
	private static void usefulMethods() {
		...
		//	변환 메서드
		System.out.println("toUpperCase: " + str.toUpperCase());	//	전부 대문자로 바꿈
		System.out.println("toLowerCase: " + str.toLowerCase());	//	전부 소문자로 바꿈
		
		//	검색 
		System.out.println("charAt: " + str.charAt(5));		//	5번 인덱스의 글자
		int index = 0;	//	인덱스 저장 변수
		index = str.indexOf("Programming");		//	str 내의 "Programming" 의 위치 인덱스
		...
		index += "Programming".length();	//	검색 시작 위치 뒤로 이동
		index = str.indexOf("Programming", index);		//	새로운 시작 위치에서 검색
		System.out.println("3rd search: " + index);		//	검색할 것이 없으면 -1;;
		//	indexOf 로 검색 시 찾는 것이 없으면 -1;;
		
		index = str.lastIndexOf("Programming");		//	검색을 뒤에서부터 실행
		...
		//	WhiteSpace (공백, 개행, 댑문자) 지우기
		String str2 = "				Hello				";
		String str3 = ", Java";
		String str4 = str2.trim();	//	WhiteSpace 지우기
		...
		//	구분자로 분리하는 법: split()
		String[] split = str.split(" "); 	//	공백 문자 기준으로 문자열을 분리, 문자열 배열로 변환
