...
/*
	함수형 프로그래밍:
	데이터를 변경시키지 않고(불변), 작업 수행 규칙(함수)을 중심으로 프로그램을 작성하는 방식
	...
	스트림:
	- 데이터를 물결의 흐름으로 파악하고, 여러 함수의 로직의 조합을 거치며 변경되고 변환되는 절차를 일컫는 말.
	...
	*/
	public static void main(String[] args) {
		...
		System.out.println(hof.getResult(20, 20));
		System.out.println(testHOF().getResult(50, 50));
	}

	public static void testForLambda() {
		//	익명 즉석 함수를 사용한 호출
		...
		BiFunction<	Integer,	//	첫 번째 입력 파라미터의 타입
					Integer,	//	두 번째 입력 파라미터 타입
					Integer		//	리턴 타입
					>findMax = (x, y) -> x > y ? x : y ;
		//	자바에서 미리 제공된 인터페이스를 사용할 때, .apply 메서드로 호출할 수 있음
		System.out.println(findMax.apply(20, 10));			
	}
	
	public static void testLambdaParams(int x, int y, MyFunction f) {
